#!/usr/bin/env python3
"""P5-v5 视频段 VLM 审查（qwen3.8-max）：六维——动作/角色/道具/招牌/画风/空间稳定性。

CLI:
  python p5_v5_video_review.py --video <mp4> --first <shotN_first.png> --out <review.json> [--frames 4]
...
审查维度（对应任务书 v5）：
  ...
  6) spatial_stability  空间稳定性：锅/柜台/玻璃门/招牌等固定物体的位置跨帧不漂移、不扭曲。
pass = score>=70 且六维全部为 true。
角色参考用本镜头首帧关键帧。
"""
...
REVIEW_PROMPT = (
    ...
    "无写实摄影残留、无风格跳变；\n"
    "6) 空间稳定性：画面中固定物体（关东煮锅、柜台、玻璃门、招牌、收银机等）的位置"
    "跨帧是否稳定不漂移、不扭曲、不重新排布（这是本次审查的重点）。\n"

#!/usr/bin/env python3
"""P5-v2 后处理（治质感）：
  1. RIFE 补帧 24->48fps：ComfyUI Frame-Interpolation（RIFE VFI, rife49.pth, multiplier=2）
  2. 放大 1344x768 -> 1920x1080：ffmpeg lanczos（B 站横屏规格）
  3. 音频回混：final_v2.mp4 音轨 copy 回 1080p 成片 -> output/out/final_v2_1080p.mp4

说明：放大用 lanczos 兜底（任务书明确允许）。RealESRGAN_x4plus 为 4x 模型，
  对 48fps 全程 ~744 帧做 4x(->5376x3072)再缩回 1920x1080 计算量过大（约 50+ 分钟），
  且 1344->1920 仅 1.43x 放大，lanczos 更干净、无 AI 伪细节。若需更锐可换 RealESRGAN。

CLI:
  python p5_v2_post.py rife --video <mp4> --out <mp4>          # RIFE 补帧
  python p5_v2_post.py finalize --rife <mp4> --audio <mp4> --out <mp4>  # 放大+回混
"""
...
def main(argv=None) -> int:
    ...
    p1.add_argument("--prefix", default="p5v2_rife")

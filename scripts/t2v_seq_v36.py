#!/usr/bin/env python3
"""v3.6 T2V 串行生成：从 v35 prompt 包读取 prompts，批量提交 H3 T2V 队列。

vs t2v_seq_v32.py 关键差异：

- 读 v36 clips_dir 的 prompt 文件（来自 prompt_pack_v35.py）
- 沿用 v32 H3 workflow（UNET = minimax_h3_fl2va_pruned_int8_convrot.safetensors）
- 失败换 seed 最多 3 次（任务书硬性指令）
- 提交前查 ComfyUI 队列深度，避免阻塞

CLI:
  python t2v_seq_v36.py --start 1 --end 6
  python t2v_seq_v36.py --no-skip --retries 3
"""
...
CLIPS_DIR = ROOT / "output" / "pipeline_v36" / "clips"
...
def gen_one_v36(shot_idx: int, prompt_path: Path, meta_path: Path,
                out_path: Path, *,
    ...
    t0 = time.time()
    print(f"[gen-v36] shot{shot_idx:02d} 开始 seed={seed} length={length} "
          f"size={width}x{height} steps={steps}", flush=True)
    print(f"[gen-v36] prompt 长度 {len(prompt)} chars", flush=True)

    first_frame_name = upload_image(seed_frame)
    print(f"[gen-v36] blank 已上传: {first_frame_name}", flush=True)

    prefix = f"v36_t2v_shot{shot_idx:02d}"
    wf = build_h3_workflow(prompt, first_frame_name, width, height, length, seed, prefix, steps)
    pid = queue_workflow(wf)
    print(f"[gen-v36] 已排队 prompt_id={pid}", flush=True)

    entry = poll_history(pid, log_prefix=f"shot{shot_idx:02d} ")
    fn, sub, vtype = find_video_output(entry)
    print(f"[gen-v36] 输出: {fn} (sub={sub!r}, type={vtype!r})", flush=True)

    download_video(fn, sub, vtype, out_path)
    dt = time.time() - t0
    print(f"[gen-v36] 完成 shot{shot_idx:02d} -> {out_path.name} 耗时 {dt:.1f}s",
          flush=True)

    qa_dir = CLIPS_DIR / "qa" / "selfcheck"
    fc_path = qa_dir / f"shot{shot_idx:02d}_filmstrip_v36_6.jpg"
    ...
    meta.update({
        ...
        "method": "t2v_blank_first_frame_v36",
...
def run_serial(start: int, end: int, *, skip_existing: bool = True,
    ...
    for shot_idx in range(start, end + 1):
        ...
        if not prompt_path.exists():
            print(f"[batch-v36] shot{shot_idx:02d} prompt 不存在 {prompt_path}, 跳过",
                  flush=True)
            results[shot_idx] = {"ok": False, "error": "prompt missing"}
            continue
        if not meta_path.exists():
            print(f"[batch-v36] shot{shot_idx:02d} meta 不存在 {meta_path}, 跳过",
                  flush=True)
            results[shot_idx] = {"ok": False, "error": "meta missing"}
            continue

        if skip_existing and out_path.exists() and out_path.stat().st_size > 100_000:
            print(f"[batch-v36] shot{shot_idx:02d} 已存在 {out_path.stat().st_size//1024} KB, 跳过",
                  flush=True)
        ...
        while attempt <= max_retries:
            try:
                ...
                if attempt > 0:
                    ...
                    print(f"[batch-v36] shot{shot_idx:02d} 第 {attempt+1} 次尝试（换 seed={seed_override}）",
                          flush=True)
                meta = gen_one_v36(shot_idx, prompt_path, meta_path, out_path,
                                   seed_override=seed_override)
                dt = time.time() - t0
                results[shot_idx] = {"ok": True, "path": str(out_path),
                                     "elapsed_sec": round(dt, 1), "meta": meta}
                print(f"[batch-v36] shot{shot_idx:02d} 完成 累计 {time.time()-t_total:.1f}s",
                      flush=True)
                last_err = None
                break
            except Exception as e:  # noqa: BLE001
                last_err = e
                print(f"[batch-v36] shot{shot_idx:02d} 第 {attempt+1} 次失败: {e}",
                      flush=True)
                attempt += 1

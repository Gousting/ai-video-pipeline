...
REVIEW_PROMPT = (
    ...
    "2) 画面中的便利店招牌/灯牌是否为纯色发光霓虹灯牌。注意：纯色发光边框上点缀的"
    "【抽象圆形光点/装饰】不算文字，只有出现【可读文字、数字、字母或乱码字符】才算违例；\n"
    "3) 道具连续性：手中零钱/放回柜台的纸币是否为【一张绿色五元纸币 + 几枚硬币】，"
    "是否出现硬币变纸币、是否出现纸币变红色；\n"
    "4) 空间稳定性（本次重点）：请聚焦【每个镜头内部】固定物体（关东煮锅、柜台、玻璃门、"
    "招牌、收银机等）的位置在其镜头内跨帧是否稳定不漂移、不扭曲、不重新排布（推近运镜导致的"
    "均匀放大不算漂移）。跨镜头之间的招牌配色差异属于分镜连续性瑕疵，不判定为空间错乱，"
    "请单独在 continuity_note 里说明。\n"
    ...
    '"spatial_stability": <true/false>, "spatial_comment": "<镜头内空间稳定性说明>", '
    '"continuity_note": "<跨镜头分镜连续性瑕疵说明，无则填无>", "opinion": "<一句话结论>"}'
)

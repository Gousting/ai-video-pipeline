#!/usr/bin/env python3
"""P5-v4 关键帧首尾一致性 VLM 审查（qwen3.8-max）+ 新海诚动画风确认。

CLI:
  python p5_v4_frame_review.py --first <first.png> --last <last.png> --out <review.json>

审查维度：
  1) 同一角色 / 同一场景 / 同一构图（仅动作状态或机位距离不同）；
  2) 两张帧均为新海诚动画电影风（无写实残留、无风格跳变）。
pass = score>=70 且 same_character 且 same_scene 且 anime_style。
"""
...
REVIEW_PROMPT = (
    ...
    "请判断：(1) 这两张图是否是【同一角色、同一场景、同一构图】，"
    "仅动作状态或机位距离不同（尾帧是首帧动作的延续，而不是换人/换场景/换视角）；\n"
    "(2) 两张图是否都是【新海诚动画电影风】（唯美细腻光影、清新通透色调、"
    "雨夜霓虹倒映、层次天空、雨丝光斑），而非写实摄影风、3D 渲染风或其它画风。\n"
    "只返回一个 JSON 对象（不要 markdown 代码块、不要额外文字），字段严格如下：\n"
    '{"score": <0-100 整数>, "same_character": <true/false>, "same_scene": <true/false>, '
    '"same_composition": <true/false>, "anime_style": <true/false>, '
    '"style_comment": "<两张图是否均为新海诚动画风的说明>", '
    '"action_progression": "<动作是否连贯延续的说明>", '

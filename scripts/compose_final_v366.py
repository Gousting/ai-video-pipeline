#!/usr/bin/env python3
"""v3.6.6 合并成片: 方案A 链式片段直接 concat + 统一 BGM 配音。

任务书 oc_task_v366.txt §6:
- 拼接: 横屏 1344x576 / yuv420p / 24fps
- 方案A: 链式片段直接 concat (画面已连续, 不需要 dissolve)
- 备用: 方案B dissolve xfade 0.06s MARGIN (链式失效时降级)
- BGM: 全部片段生成完成后, 统一铺 bgm_v32.wav (120BPM) 在整个 40s 成片上
- 不在每个片段单独配音 (否则音乐随片段硬切换, 破坏整体感)

vs compose_final_v365.py 关键差异:
- 输入 clips_v366/ (v366 链式生成的 6 段)
- 输出 final_v36_60s_v366.mp4
- 横屏 1344x576 (非竖屏 768x1344)
- 拼接: 直接 concat (默认) 或可选 dissolve xfade (备用)
- BGM: 统一铺底 (沿用 mix_bgm)

CLI:
    python compose_final_v366.py
    python compose_final_v366.py --with-dissolve
    python compose_final_v366.py --dry-run
"""
...
DEFAULT_RAW_DIR = ROOT / "output" / "pipeline_v36" / "clips_v366"
DEFAULT_BGM = ROOT / "output" / "pipeline_v3" / "music" / "bgm_v32.wav"
DEFAULT_OUT = ROOT / "output" / "pipeline_v36" / "final_v36_60s_v366.mp4"
DEFAULT_TMP = ROOT / "output" / "pipeline_v36" / "tmp" / "v366"
DEFAULT_RHYTHM = ROOT / "output" / "pipeline_v36" / "shots_v366" / "rhythm_plan_v366.json"
...
def run(cmd: list[str], *, dry_run: bool = False, check: bool = True) -> str:
    ...
    if dry_run:
        ...
        return s
    print(f"[compose-v366] + {s}", flush=True)
...
def concat_all(clips: list[Path], out_path: Path,
    ...
    """直接 concat demuxer (方案A: 链式片段画面已连续, 不需要 xfade)。"""
...
def concat_with_xfade(clips: list[Path], out_path: Path, *,
                      xfade_dur: float = 0.3,
                      safe_gap: float = SAFE_GAP_SEC,
                      dry_run: bool = False) -> float:
    """方案B: 链式片段 + 段间 dissolve xfade 留 MARGIN (备用, 默认不开)。"""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    n = len(clips)
    durations = [ffprobe_stream_duration(c) for c in clips]

    filter_parts = []
    for i, c in enumerate(clips):
        filter_parts.append(
            f"[{i}:v]format=yuv420p,scale={RES_W}:{RES_H}:flags=lanczos:"
            f"force_original_aspect_ratio=decrease,"
            f"pad={RES_W}:{RES_H}:(ow-iw)/2:(oh-ih)/2:black,"
            f"setsar=1:1,fps={FPS}[v{i}]"
        )

    # 真实链路是: [v0][v1]xfade->[m1]; [m1][v2]xfade->[m2]; ...; [mN-1] 命名 [vout]
    filter_complex = ";".join(filter_parts)
    chain_parts = ["[v0]"]
    for i in range(1, n):
        if i == 1:
            offset_i = durations[0] - xfade_dur - safe_gap
        else:
            # 上一段 chain 累计时长 = sum(durations[0:i]) - (i-1) * (xfade_dur + safe_gap) ?
            # 实际 ffmpeg offset 单位是秒, 表示 xfade 在前一链上的时间戳
            offset_i = (sum(durations[:i])
                        - (i - 1) * (xfade_dur + safe_gap)
                        - xfade_dur - safe_gap)
        out_label = "[vout]" if i == n - 1 else f"[m{i}]"
        chain_parts.append(
            f"[v{i}]xfade=transition=dissolve:duration={xfade_dur:.3f}:"
            f"offset={offset_i:.3f}{out_label}"
        )
    chain_str = "".join(chain_parts)
    filter_complex += ";" + chain_str
...
def main(argv=None) -> int:
    ...
    print(f"[compose-v366] {len(clip_paths)} shots:", flush=True)
    for p, d in zip(clip_paths, durations):
        print(f"  {p.name} {d:.3f}s", flush=True)

    if args.with_dissolve:
        print("[compose-v366] 方案B (dissolve xfade): 备用模式",
              flush=True)
        # xfade 链路
        concat_v = tmp_dir / "chain_xfade.mp4"
        concat_dur = concat_with_xfade(clip_paths, concat_v,
                                       xfade_dur=0.3,
                                       dry_run=args.dry_run)
        method = "xfade_with_margin"
    else:
        print("[compose-v366] 方案A (链式直接 concat): 默认模式",
              flush=True)
        concat_v = tmp_dir / "concat_all.mp4"
        concat_dur = concat_all(clip_paths, concat_v,
                                dry_run=args.dry_run)
        method = "concat_demuxer"

    print(f"[compose-v366] concat 完成: {concat_dur:.3f}s ({concat_v.name})",
          flush=True)
    ...
    if bgm_path and bgm_path.exists():
        ...
        print(f"[compose-v366] WARN: BGM not found, output is silent",
              file=sys.stderr)

    print(f"[compose-v366] final output → {out_path}", flush=True)

    meta = {
        "compose_phase": "compose_final_v366",
        ...
        "ffmpeg_cmds": [
            {"step": "concat", "method": method,
             "out": str(concat_v), "out_dur": round(concat_dur, 4)},
            {"step": "bgm_mix", "cmd": bgm_cmd},
        ],
        "vs_v365_changes": {
            "horizontal_2_36_1": True,
            "no_in_prompt_transition": True,
            "chain_first_frame_i2v": True,
    ...
    meta_path.write_text(json.dumps(meta, ensure_ascii=False, indent=2),
                          encoding="utf-8")
    print(f"[compose-v366] meta → {meta_path}", flush=True)
    return 0
